" reStructuredText tables plugin 0.5
" Language:     Python (ft=python)
" Maintainer:   Vincent Driessen <vincent@datafox.nl>
" Version:      Vim 7 (may work with lower Vim versions, but not tested)
" URL:          http://github.com/nvie/vim-rst-tables
"
" Modified by Daan Bakker to be automatic, and an autoload plugin
"
...
fun! rst_tables#reformat()
  let top = line('.')
  while getline(top-1)!~'^\s*$'
    let top -= 1
  endw
  let bot = line('.')
  while getline(bot+1)!~'^\s*$'
    let bot += 1
  endw
  if ((getline(top)=~'^+-.*+$' && getline(bot)=~'^+-.*+$') || getline('.')=~'^\s*|.*|') && !exists('b:norealign') && !exists('g:norealign')
    exe 'python reformat_table('.top.', '.bot.')'
  endif
endf
...
def reformat_table(upper, lower):
    slice = vim.current.buffer[upper - 1:lower]
    indent = re.match('^\s*', slice[0]).group(0)
